...
@property (nonatomic) Secondmodel *model;
...
{
    //_model = [[Secondmodel alloc]init];
    _imageNameArray = imageNameArray;
    _model = _imageNameArray[0];
    [_leftImageView sd_setImageWithURL:[NSURL URLWithString:_model.img]];
    NSLog(@"%@",_model.img);
    _model = _imageNameArray[1];
    
    [_centerImageView sd_setImageWithURL:[NSURL URLWithString:  _model.img]];
    NSLog(@"%@",_model.img);
    _model = _imageNameArray[2];
    [_rightImageView sd_setImageWithURL:[NSURL URLWithString:  _model.img]];
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    _model = _imageNameArray[(currentImage-1)%_imageNameArray.count];
    [_leftImageView  sd_setImageWithURL:[NSURL URLWithString: _model.img]];
    
    _leftAdLabel.text = _adTitleArray[(currentImage-1)%_imageNameArray.count];
    _model = _imageNameArray[currentImage%_imageNameArray.count];
    [_centerImageView sd_setImageWithURL:[NSURL URLWithString: _model.img]];
